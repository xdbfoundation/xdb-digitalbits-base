...
XDR_BASE_URL_NEXT=https://github.com/xdbfoundation/digitalbits-xdr-next/raw/main
...
XDR_FILES_NEXT= \
	...
	DigitalBits-transaction.x \
	DigitalBits-types.x \
	DigitalBits-contract.x \
	DigitalBits-contract-env-meta.x \
	DigitalBits-contract-spec.x
...
src/generated/curr_generated.js: $(XDR_FILES_LOCAL_CURR)
	...
	docker run -it --rm -v $$PWD:/wd -w /wd ruby /bin/bash -c '\
		...
		gem specific_install https://github.com/xdbfoundation/xdrgen.git -b $(XDRGEN_COMMIT) && \
...
types/curr.d.ts: src/generated/curr_generated.js
	docker run -it --rm -v $$PWD:/wd -w / --entrypoint /bin/sh node:alpine -c '\
		apk add --update git && \
		git clone --depth 1 https://github.com/xdbfoundation/dts-xdr -b $(DTSXDR_COMMIT) --single-branch && \
		cd /dts-xdr && \
		yarn install && \
