...
describe("Memo.text()", function() {
  ...
  it("returns a value for a correct argument (utf8)", function() {
    let memoText = DigitalBitsBase.Memo.text([0xd1]).toXDRObject().toXDR();
    ...
    memoText = DigitalBitsBase.Memo.text(Buffer.from([0xd1])).toXDRObject().toXDR();
    expect(memoText.equals(expected)).to.be.true;
  });
  ...
  it("throws an error when invalid argument was passed", function() {
    expect(() => DigitalBitsBase.Memo.text()).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => DigitalBitsBase.Memo.text({})).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => DigitalBitsBase.Memo.text(10)).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => DigitalBitsBase.Memo.text(Infinity)).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => DigitalBitsBase.Memo.text(NaN)).to.throw(/Expects string, array or buffer, max 28 bytes/);
  });

  it("throws an error when string is longer than 28 bytes", function() {
    expect(() => DigitalBitsBase.Memo.text("12345678901234567890123456789")).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => DigitalBitsBase.Memo.text("三代之時三代之時三代之時")).to.throw(/Expects string, array or buffer, max 28 bytes/);
  });
