...
describe('Transaction', function() {
  ...
  it("constructs Transaction object from a TransactionEnvelope", function(done) {
    let source      = new DigitalBitsBase.Account("GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB", "0");
    ...
    let input = new DigitalBitsBase.TransactionBuilder(source)
                .addOperation(DigitalBitsBase.Operation.payment({destination, asset, amount}))
    ...
    var transaction = new DigitalBitsBase.Transaction(input);
  ...
  it("does not sign when no Network selected", function() {
    ...
    let asset       = DigitalBitsBase.Asset.native();
    let amount      = "2000";
    let signer      = DigitalBitsBase.Keypair.random();
  ...
  it("accepts 0 as a valid transaction fee", function(done) {
    ...
    let input = new DigitalBitsBase.TransactionBuilder(source, {fee: 0})
                .addOperation(DigitalBitsBase.Operation.payment({destination, asset, amount}))
                .addMemo(DigitalBitsBase.Memo.text('Happy birthday!'))
                .build()
                .toEnvelope()
